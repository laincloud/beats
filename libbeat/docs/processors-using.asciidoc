 ...
 * <<parse_regex_fields, `parse_regex_fields`>>
...
The `parse_regex_fields` processor can parse the `message` field of the event by the `regexp` parameter if the associated condition is fulfilled. It can also capture the submatches as the root fields. The `regexp` parameter is mandatory. The regular expression must be accepted by the `RE2` syntax.
...
If the `message` field matches `regexp`. Field `remote_addr`, `remote_user`, `time_local` and `request_host` will be in the new event.